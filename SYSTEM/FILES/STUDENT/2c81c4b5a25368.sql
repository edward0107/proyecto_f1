...
CREATE TABLE documento_alumno (
    ...
    docal_url_documento VARCHAR(500),
...
CREATE TABLE grado(
    ...
    grad_nivel INT,
    grad_grado VARCHAR(250), 
    FOREIGN KEY (grad_nivel) REFERENCES nivel(niv_id)
)

CREATE TABLE seccion(
    sec_id INT PRIMARY KEY,
    sec_grado INT,
    sec_seccion VARCHAR(100),
    FOREIGN KEY (sec_grado) REFERENCES grado(grad_id)
)